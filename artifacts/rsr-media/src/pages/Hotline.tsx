...
import { Phone, Mail, AlertTriangle, Shield, Radio, FileText, Users, Mic } from 'lucide-react';
...
const USE_CASES = [
  { icon: Radio, label: "CALL IN DURING THE SHOW", desc: "When live broadcasts are active, the hotline accepts call-ins and viewer questions. Callers may be put on air at the host's discretion. Calling does not guarantee broadcast use." },
  { icon: FileText, label: "SUBMIT A TIP", desc: "Off-air tips, story leads, and community concerns can be submitted by phone. Voicemails are reviewed. You may also use the written form below or email the newsroom." },
  { icon: AlertTriangle, label: "REQUEST A CORRECTION", desc: "If RSR Media published something that needs correction, call or write. Corrections are reviewed and published when warranted. Include the report URL and the specific error." },
  { icon: Users, label: "COMMUNITY CONCERN", desc: "Local issues, infrastructure problems, public accountability questions, or institutional misconduct — anything the public should know about is worth a call or tip." },
  { icon: Mic, label: "INTERVIEW / GUEST INQUIRY", desc: "Requesting to be a guest, comment on a story, or be interviewed? Use the written form or email the newsroom with your name, topic, and contact info." },
];
...
export default function Hotline() {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TipForm>({
  ...
  function onSubmit(data: TipForm) {
    ...
    const typeLabels: Record<string, string> = {
      tip: 'TIP SUBMISSION', correction: 'CORRECTION REQUEST',
      concern: 'COMMUNITY CONCERN', interview: 'INTERVIEW/GUEST INQUIRY',
    };
    const subject = encodeURIComponent(`RSR HOTLINE — ${typeLabels[data.tipType]} — ${data.topic}`);
    const body = encodeURIComponent(
      [
        `RSR HOTLINE SUBMISSION`,
        `TYPE: ${typeLabels[data.tipType]}`,
        `---`,
        `TOPIC: ${data.topic}`,
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                <a
                  ...
                  className="inline-flex items-center gap-2 font-serif font-bold tracking-widest uppercase h-11 px-6 text-[0.7rem] border border-primary/50 text-primary bg-primary/10 hover:bg-primary hover:text-primary-foreground transition-all corner-bracket"
                >
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <p className="font-mono text-[0.65rem] text-muted-foreground/50 tracking-widest">
          ...
          <div className="space-y-5">
            ...
            <div className="border border-border/40 bg-card/15 p-5 corner-bracket">
              ...
              <p className="font-sans text-sm text-muted-foreground leading-relaxed">
                RSR Media accepts tips on local community concerns, public accountability, infrastructure, institutional misconduct, and public-interest stories of all kinds.
              </p>
            </div>

            <div className="border border-border/40 bg-card/15 p-5 corner-bracket">
              <div className="font-mono font-bold text-xs text-muted-foreground tracking-widest uppercase mb-2">READ THE REPORTS</div>
