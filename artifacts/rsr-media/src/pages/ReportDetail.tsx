...
export default function ReportDetail() {
  ...
  const related = REPORTS.filter(
    r => r.id !== report.id && r.status === 'published' && r.tags.some(t => report.tags.includes(t))
  ).slice(0, 3);
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <article className="lg:col-span-2">
            <div className="flex flex-wrap items-center gap-3 mb-6 font-mono text-xs tracking-widest uppercase">
              ...
              <time className="text-muted-foreground">{new Date(report.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
              {report.updatedAt !== report.date && (
                <span className="text-muted-foreground/60">Updated {new Date(report.updatedAt).toLocaleDateString()}</span>
              )}
            </div>
            ...
            <div className="glass-panel border border-border/40 p-8 md:p-10">
              <div className="prose prose-sm prose-invert max-w-none font-sans leading-relaxed text-muted-foreground whitespace-pre-line">
                {report.body}
              </div>
            </div>
            ...
            {related.length > 0 && (
              <div className="mt-16">
                ...
                <div className="space-y-4">
                  {related.map(r => (
                    <Link key={r.id} href={`/reports/${r.slug}`} className="block p-4 border border-border/40 bg-card/20 hover:border-primary/40 hover:bg-card/40 transition-colors corner-bracket">
          ...
          <aside className="space-y-6">
                  ...
                  {report.sourceLinks.map((s, i) => (
                    <li key={i}>
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-start gap-2 font-mono text-xs text-muted-foreground hover:text-foreground transition-colors"
                      >
            ...
            {report.tags.length > 0 && (
              <div className="border border-border/50 bg-card/20 p-5 corner-bracket">
                ...
                <div className="flex flex-wrap gap-2">
                  {report.tags.map(t => (
                    <span key={t} className="font-mono text-[0.65rem] tracking-wider border border-border px-2 py-0.5 text-muted-foreground uppercase">
                      {t}
                    </span>
                  ))}
                </div>
            ...
            <div className="border border-border/50 bg-card/20 p-5 corner-bracket">
              ...
              <p className="font-sans text-xs text-muted-foreground leading-relaxed mb-4">Submit tips related to this topic or future reporting.</p>
            ...
            <div className="border border-border/50 bg-card/20 p-5 corner-bracket">
              ...
              <a
                href={`mailto:newsroom@rsrmedia.org?subject=Correction Request — ${encodeURIComponent(report.title)}`}
